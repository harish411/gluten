...
function process_setup_al2023 {
  # AL2023 does not ship gcc-toolset-12 (RHEL SCL); use the native gcc/g++ instead.
  sed -i "s/ninja-build cmake gcc-toolset-12/ninja-build cmake gcc gcc-c++/" ${CURRENT_DIR}/setup-rhel.sh
  sed -i "s#source /opt/rh/gcc-toolset-12/enable || exit 1#export CC=gcc CXX=g++#g" ${CURRENT_DIR}/setup-rhel.sh
  # AL2023 ships cmake 3.22 via dnf; skip the pip cmake install (pip SSL fails outside AWS).
  sed -i "/^[[:space:]]*pip install cmake/s/^/#/" ${CURRENT_DIR}/setup-rhel.sh
}
...
function setup_linux {
  ...
  elif [[ "$LINUX_DISTRIBUTION" == "rhel" || "$LINUX_DISTRIBUTION" == "amzn" ]]; then
    case "$LINUX_VERSION_ID" in
      9.6) ;;
      9.7) ;;
      2023) ;;
      *)
        echo "Unsupported rhel/amzn version: $LINUX_VERSION_ID"
        exit 1
      ;;
    esac
  else
